...
class Resume extends CI_Controller
{
	...
	{
		if (!file_exists('application/views/resume/edit.php'))
		{
			// Whoops, we don't have a page for that!
			show_404();
		}
		...
		if ($this->form_validation->run() === FALSE)
		{
			$aResume['aAsphalt'] = $this->asphalt_model->find($iAsphaltId);
			$this->layout->css(array(base_url().'public/css/general.css'));
			$this->layout->setTitle('Sistema control asfalto | Resumen');
			$this->layout->view('edit',$aResume);
		}
		else
		{
			$this->asphalt_model->update($iAsphaltId);
			$this->layout->view('success');
		}		
	}
